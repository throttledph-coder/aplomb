...
// Animated WebGL2 smoke background (adapted from a 21st.dev shader). The brightest
// noise is tinted with `smokeColor`. GPU-light (single full-screen quad).
...
export function SmokeBackground({ smokeColor = '#d97757', className }: SmokeBackgroundProps) {
  ...
  useEffect(() => {
    ...
    // Always animate — the smoke is gentle + non-flashing (OS "reduce motion"
    // would otherwise freeze it on Windows where it's commonly off).
    let raf = 0
